...
function windowScroll(){
    ...
    var ws_end = $('section.editor .inner').innerHeight()-$('section.editor .inner .links').innerHeight()-$('.float').innerHeight();

    if(winScroll<$('section.editor').offset().top-210){
        $('.float').stop().animate({"top":0},0,"linear");
    }else if(winScroll>=$('section.editor').offset().top-210 && winScroll<$('section.editor .inner .links').offset().top-430){
        $('.float').stop().animate({"top":ws},0,"linear");
    }else if(winScroll>=$('section.editor .inner .links').offset().top-430){
